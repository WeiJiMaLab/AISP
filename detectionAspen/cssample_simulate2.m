function resp = cssample_simulate2(x,dMat,logflag)
...
log_p = sum(kappa_x .* cos(x - prop_xi) + kappa_y .* cos(y - prop_xi - DeltaMat), 2);
...
for isamp = 1:nSamples
    ...
    log_p_new = sum(kappa_x .* cos(x - prop_xi) + kappa_y .* cos(y - prop_xi + prop_DeltaMat), 2);
